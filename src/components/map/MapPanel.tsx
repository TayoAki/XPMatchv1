...
export function MapPanel() {
  ...
  const tripId = tripScope ?? view.tripId;
        ...
        >
          ...
          <div className="absolute left-4 top-4 flex items-center gap-2">
            ...
            {searchOpen ? (
              ...
              >
                ...
                {focus.name}
                {visiblePlaces.length ? <span className="text-[12px] font-medium text-muted">{visiblePlaces.length} pinned</span> : null}
              </button>
            ) : null}
          </div>
          ...
          {stripVisible ? (
            <div className="absolute inset-x-0 bottom-0 z-[5] bg-gradient-to-t from-white/95 via-white/80 to-transparent pt-4">
              <PinStrip places={visiblePlaces} selectedKey={selectedKey} hoveredKey={hoveredKey} onSelect={onSelect} onHover={onHover} />
            </div>
          ) : null}
