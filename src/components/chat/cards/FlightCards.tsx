...
export function FlightCards({ args, status }: { args: Streaming<ShowFlightsArgs>; status: ToolCallStatus }) {
  ...
  return (
    <div>
      <SectionHeader
        title={args.destination ? `Flights ${args.origin ? `${args.origin} → ` : ""}${args.destination}` : "Flights"}
        subtitle={[dates, args.travelers ? `${args.travelers} travelers` : "", "Prices are estimates"].filter(Boolean).join(" · ")}
        status={status}
      />
      <CardGrid>
        {items.map((o, i) => (
          <CardShell key={`${o.airline}-${i}`}>
            <Body>
              <div className="flex items-start justify-between gap-2">
                ...
                <SaveButton
                  kind="flight"
                  title={`${o.airline} ${o.routeSummary ?? ""}`.trim()}
